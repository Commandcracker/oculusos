local input_colour, output_colour, text_colour, keyword_colour, comment_colour, string_colour =
    ...
    colours.grey,
    colours.red
local number_colour, extra_colour, object_colour = colours.magenta, colours.grey, colours.lightGrey

local keywords = {
    ["and"] = keyword_colour,
    ["break"] = keyword_colour,
    ["do"] = keyword_colour,
    ["else"] = keyword_colour,
    ["elseif"] = keyword_colour,
    ["end"] = keyword_colour,
    ["false"] = object_colour,
    ["for"] = keyword_colour,
    ["function"] = keyword_colour,
    ["if"] = keyword_colour,
    ["in"] = keyword_colour,
    ["local"] = keyword_colour,
    ["nil"] = object_colour,
    ["not"] = keyword_colour,
    ["or"] = keyword_colour,
    ["repeat"] = keyword_colour,
    ["return"] = keyword_colour,
    ["then"] = keyword_colour,
    ["true"] = object_colour,
    ["until"] = keyword_colour,
    ["while"] = keyword_colour
}

local tokens = {
    {"^%s+", text_colour},
    -- Identifiers and keywords
    {"^[%a_][%w_]*", function(match)
            return keywords[match] or text_colour
        end},
...
local function pretty_sort(a, b)
    ...
    if ta == "string" then
        return tb ~= "string" or a < b
    elseif tb == "string" then
        return false
    end

    if ta == "number" then
        return tb ~= "number" or a < b
    end
    return false
end
...
local function pretty_impl(obj, tracking, width, height, indent, tuple_length)
    ...
    elseif width <= 7 then
        write_with(text_colour, open)
        write_with(extra_colour, " ... ")
        write_with(text_colour, close)
        return
    end
    ...
    for k = 1, length do
        if not first then
            write_with(text_colour, next_newline)
        else
            first = false
        end
...
local function pretty(t, n)
    ...
    local fit_height = true--settings.get("mbs.lua.pretty_height", true)
    if type(fit_height) == "number" then
        height = fit_height
    elseif fit_height == false then
        height = 1 / 0
    end
...
local function process_auto_run_file(folderPath, file)
    ...
    local ok, result
    if true then --settings.get("mbs.lua.traceback", true)
...
local history_file = ".lua_history"--settings.get("mbs.lua.history_file", ".lua_history")
...
while running do
    ...
    local line
    if readline and readline.read then --and settings.get("mbs.lua.highlight")
        line =
            readline.read {
            history = history,
    ...
    if line:find("%S") then
        if line ~= history[#history] then
            ...
            local max = 1e4 -- tonumber(settings.get("mbs.lua.history_max", 1e4)) or 1e4
