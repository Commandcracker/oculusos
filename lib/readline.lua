...
function read(opts)
    ...
    local complete_fg = opts.complete_fg or colour_table["grey"] or -1
    local complete_bg = opts.complete_bg or colour_table["none"] or -1
    ...
    local function updateModifier()
        nMod = 0
        if tDown[keys.leftCtrl] or tDown[keys.rightCtrl] then
            nMod = nMod + 1
        end
        if tDown[keys.leftAlt] or tDown[keys.rightAlt] then
            nMod = nMod + 2
        end
    end
    ...
    local function redraw(_bClear)
        ...
        if opts.highlight and not _bClear then
            ...
            while hl_pos <= hl_max do
                ...
                if next_pos < hl_pos then
                    error("Highlighting function consumed no input")
                end

                if next_pos >= nScroll + 1 then
                    if next_col ~= hl_col then
                        term.setTextColor(next_col)
                        hl_col = next_col
                    end
                    if sReplace then
        ...
        if nCompletion then
            ...
            if not _bClear then
                ...
                if complete_fg > -1 then
                    term.setTextColor(complete_fg)
                end
                if complete_bg > -1 then
                    term.setBackgroundColor(complete_bg)
                end
            end
            if sReplace then
    ...
    while true do
        ...
        elseif sEvent == "key" then
            ...
            elseif
                nMod == 0 and (param == keys.up or param == keys.down) or
                    nMod == 1 and (param == keys.p or param == keys.n)
            then
                -- Up or down
                if nCompletion then
                    -- Cycle completions
            ...
            elseif nMod == 1 and param == keys.t then
                ...
                local prev, cur
                if nPos == #sLine then
                    prev, cur = nPos - 1, nPos
                elseif nPos == 0 then
                    prev, cur = 1, 2
                else
                    prev, cur = nPos, nPos + 1
                end
            ...
            elseif nMod == 2 and param == keys.c then
                ...
                if nPos < #sLine then
                    ...
                    sLine =
                        nsub(1, nPos) ..
                        nsub(nPos + 1, nPos + 1):upper() .. nsub(nPos + 2, nNext):lower() .. nsub(nNext + 1, #sLine)
                    nPos = nNext
                    clear()
                    recomplete()
                    redraw()
